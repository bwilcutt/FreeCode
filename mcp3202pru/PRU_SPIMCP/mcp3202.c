...
// The ctrlData_s structure must match the same data structure in the PRU software.
 ...
 * from the MCP3202 immediately.  This function will also
 * calculate the pulse-per-second of each channel based on
 * the given threshold and sample rate.
 **************************************************************/
int main(void)
{
	uint16_t ch0 = 0;		// Raw sample from channel 0
	uint16_t ch1 = 0;		// Raw sample from channel 1
	uint16_t prevSample_ch0 = 0;	// Previous raw sample
	uint16_t prevSample_ch1 = 0;	// Previous raw sample
	uint16_t ch0_counter = 0;	// # of times ch0 is over threshold
	uint16_t ch1_counter = 0;	// # of times ch1 is over threshold
	uint16_t kHZCounter = 0; // @ 3.3v, 16 bits should be enough for max of 64kHz.
	...
	// Pre-init previous-read-value so we don't falsely count a pulse when there was none.

	prevSample_ch0 = get_value(ctrlData.mosiWordOut_ch0) & 0xfff;
	prevSample_ch1 = get_value(ctrlData.mosiWordOut_ch1) & 0xfff;
	...
	while (1)
	{
		if (ctrlData.readyFlag)
		{
			if (1 == ctrlData.pConfigFlag)
			{
				...
				// Calculate pulse rates here

				if (ch0 >= ctrlData.pThreshold_ch0 && prevSample_ch0 < ctrlData.pThreshold_ch0)
					ch0_counter++;

				if (ch1 >= ctrlData.pThreshold_ch1 && prevSample_ch1 < ctrlData.pThreshold_ch1)
					ch1_counter++;

				prevSample_ch0 = ch0;
				prevSample_ch1 = ch1;

				if (kHZCounter++ > ctrlData.pSampleRate)
				{
					kHZCounter = 0;

					// Record pulses here

					ctrlData.pPulsePerSec_ch0 = ch0_counter;
					ctrlData.pPulsePerSec_ch1 = ch1_counter;

					// Reset our counters for another go around

					ch0_counter = 0;
					ch1_counter = 0;
				}

				// Store raw values for running average.
...
uint16_t get_value(uint8_t mosiout)
{
	...
	SPI_ChannelSet(spiReg, SPI_CHANNEL_ENABLE, ctrlData.spiCh); // CS to high

	// MCP3202 is 24-bit access to get 12-bits of data.  Wirte 00000001b to the port as the first byte.

	outVal = 0;

	// Write data, 4 bits (or WL setting).  Note first byte must be 7 0's followed by 1 start bit.

	outVal |= (mosiout << 13);

	SPI_TransmitData32(spiReg, outVal, ctrlData.spiCh);
	...
	SPI_ChannelSet(spiReg, SPI_CHANNEL_DISABLE, ctrlData.spiCh); // CS to low
...
void init_spi(void)
{
	...
	sampleRate = (24 * (ctrlData.pSampleRate * 2));
	...
	// MCP3202 requires 24-bits.

	SPI_WordLengthSet(spiReg, SPI_WORD_LENGTH(24), ctrlData.spiCh); 
	SPI_WordCountSet(spiReg, 1);
